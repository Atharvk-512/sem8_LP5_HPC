...
__global__ void gpu_multiply (int *arr1, int *arr2, int *res, int s) {
	...
	if(block_id_x < s && block_id_y < s) {
		...
		res[(block_id_x*s) + block_id_y] = sum;
	}
}
...
// atharv@atharv-laptop-ubuntu:~/BE_sem8_HPC_lab/Assn4_CUDA$ nvcc matrix_multiplication.cu 
// atharv@atharv-laptop-ubuntu:~/BE_sem8_HPC_lab/Assn4_CUDA$ ./a.out 
// Enter size of array: 1000

// Result gpu multiplication: Elapsed Time (GPU): 68.7042
// Result cpu multiplication: Elapsed Time (CPU): 3822.7
